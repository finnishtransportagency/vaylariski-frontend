...
describe('Wayarea field works', () => {
  beforeEach(() => {
    ...
    cy.getByDataCyId('navline.VAYLAT.id').find('button').as('wayarea-dropdown-button')
    cy.get('input[id="navline.starting_gdo_gid"]').as('gid-input')
    cy.getByDataCyId('navline.starting_gdo_gid.id').find('button').as('gid-dropdown-button')
  ...
  it('Correct fields are disabled initially with correct tooltips', () => {
    ...
    cy.get('#gdo-gid-tooltip').contains('Valitse väylä ensin')
    
    //Check submit button
    cy.get('@submit-button').scrollIntoView()
    cy.get("@submit-button").should('be.disabled')
    cy.get("@submit-button").trigger('mouseover', {force: true})
    cy.get('#submit-button-tooltip').contains('Korjaa seuraavat asiat lähetettäeksi arvot:')
    cy.get('#submit-button-tooltip').contains('- VAYLAT id vaaditaan')
    cy.get('#submit-button-tooltip').contains('- Valitulle väylälle ei ole tunnuksia')
  })
  ...
  describe('Selecting a wayarea', () => {
    it('Selecting wayarea with GDO_GIDS enables GDO_GID-input with options and enables submit button', () => {
      ...
      cy.get("@wayarea-dropdown-button").click()
      cy.get('ul[id="navline.VAYLAT-listbox"]').find("li").contains("100 - Oulu - Kemi väylä").click()
      
      //Check GDO GID field
      cy.get("@gid-input").should('be.enabled')
      cy.get("@gid-dropdown-button").click()
      cy.get('ul[id="navline.starting_gdo_gid-listbox"]').find("li").contains("227903")
    ...
    it('Selecting wayarea with no GDO_GID keeps GDO_GID-input and submit button disabled and has correct tooltips', () => {
      ...
      cy.get("@wayarea-dropdown-button").click()
      cy.get('ul[id="navline.VAYLAT-listbox"]').find("li").contains("7010 - Akonniemen väylät").click()
      
      //Check notification
      cy.contains('Navigointilinjan tunnusta ei löytynyt valitulle väylälle id:llä 7010')

      //Check GDO GID field
      cy.get("@gid-input").should('be.disabled')
      cy.get("@gid-input").trigger('mouseover', {force: true})
      cy.get('#gdo-gid-tooltip').contains('Valitulle väylälle ei ole tunnuksia')
